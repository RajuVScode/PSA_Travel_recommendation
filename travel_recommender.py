...
"""
Travel Recommender Chatbot using Gradio + Azure OpenAI
...
from travelStatePipeline import generate_travel_recommendation_langgraph
...
def bot_turn(user_message: str, history: List[List[str]], state: Dict[str, Any]):
    ...
    if next_question:
        ...
        if state["intent"].is_complete():
            ...
            # )
            print("Generating travel recommendation...")
            print("User input:", assistant_message)
            assistant_text = generate_travel_recommendation_langgraph(assistant_message)
        else:
...
with gr.Blocks(title="Travel Recommender") as demo:
